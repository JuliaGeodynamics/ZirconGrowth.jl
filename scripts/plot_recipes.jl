...
Plotting recipes for ZirconGrowth results using GLMakie.
Load this file after `using GLMakie` and `using ZirconGrowth`.

Usage:
    using GLMakie, ZirconGrowth
    include("scripts/plot_recipes.jl")
    using .ZirconGrowthMakie
"""
module ZirconGrowthMakie

using GLMakie
using ZirconGrowth

export plot_trace_properties, plot_element_profiles, plot_element_ratios,
       plot_growth_diagnostics, plot_rim_evolution
...
"""
    plot_trace_properties(Th, XH2O, element_names; [fig])

Plot partition coefficients (left) and diffusion coefficients (right)
versus inverse temperature 1000/T for all trace elements and Zr.
Reproduces MATLAB `plot_trace.m`.
"""
function plot_trace_properties(Th::AbstractVector{Float64},
                               XH2O::Float64,
...
"""
    plot_element_profiles(short::SimulationResult, long::SimulationResult;
                          elements=["U","Y","P","Th","Hf","Sc"], [fig])

Plot rim-to-core zircon concentration profiles for selected elements,
comparing two runs with different durations.
"""
function plot_element_profiles(short::SimulationResult{N},
    ...
    nrow = cld(length(elements), 3)
    ncol = min(length(elements), 3)
...
"""
    plot_element_ratios(short::SimulationResult, long::SimulationResult;
                        ratios, [fig])

Plot ratios of zircon rim concentrations (e.g. Y/Th, Y/Hf, …).
"""
function plot_element_ratios(short::SimulationResult{N},
                             long::SimulationResult{N};
...
# ─── Growth-diagnostic plots (Figures 3 from single_profile.m) ───────────────

"""
    plot_growth_diagnostics(res::SimulationResult; [fig])

Four-panel diagnostic: i) Zr melt profile, ii) mass error,
iii) growth/contraction rates, iv) zircon radius vs time.
"""
function plot_growth_diagnostics(res::SimulationResult; fig::Figure = Figure(size=(900, 700)))
